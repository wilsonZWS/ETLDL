...
class Subplot(object):
    ...
    def addLine(self, label, level):
        """Add a horizontal line to the picture.
        :param label: A label.
        :type label: string.
    ...
    def getSeriesKey(self):
        try:
            return list(self._Subplot__series.keys())[0]
        except Exception:
            raise KeyError("key values is not available")
...
class StrategyPlotter(object):
    ...
    :param strat: The strategy to picture.
    ...
    def __init__(self, strat, plotAllInstruments=True, plotBuySell=True, plotPortfolio=True):
        ...
        self.__strat_name = strat.__class__.__name__
        if plotPortfolio:
    ...
    def plotSep(self, fromDateTime=None, toDateTime=None, postPlotFun=_post_plot_fun,
                dpi=None, show=False, save=False, save_path='picture/', format='png', figsize=(15, 8)):
        dateTimes = _filter_datetimes(self.__dateTimes, fromDateTime, toDateTime)
        dateTimes.sort()
        subplots = []
        subplots.extend(self.__barSubplots.values())
        ...
        dir = save_path + self.__strat_name
        if save:
            if not os.path.exists(dir):
                os.makedirs(dir)
        self.__sep_save_dir = dir

        for i, subplot in enumerate(subplots):
            fig, axes = plt.subplots(nrows=1, sharex=True, squeeze=False, figsize=figsize)
            axesSubplot = axes[0][0]
            subplot.plot(axesSubplot, dateTimes, postPlotFun=postPlotFun)
            axesSubplot.grid(True)
            fig.autofmt_xdate()
            if save:
                try:
                    filename = dir + '/' + subplot.getSeriesKey() + '.png'
                except Exception:
                    filename = dir + '/subplot_{}.png'.format(i)
    ...
    def getSepPlotDir(self):
        return self.__sep_save_dir
